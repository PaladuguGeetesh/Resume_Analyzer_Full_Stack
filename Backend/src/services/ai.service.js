...
function buildResumePdfPrompt({ resume, selfDescription, jobDescription }) {
  return `Generate resume for a candidate with the following details:
                      ...
                      IMPORTANT: The HTML must be fully self-contained, with no external resources of any kind — no <link> tags to external stylesheets or fonts (no fonts.googleapis.com, no CDNs), no <img> tags pointing at external URLs, and no external JavaScript. Use only inline <style> blocks and system-safe fonts (Arial, Helvetica, Georgia, sans-serif) for any font styling. This HTML is rendered to PDF offline, with no network access.
...
async function generatePdfFromHtml(htmlContent) {
  ...
  // TEMP DEBUG: diagnosing the Render free-tier networkidle0 timeout — remove once
  // confirmed fixed. Checks two independent hypotheses side by side: (1) the generated
  // HTML pulling in external resources that networkidle0 waits on and that stall from
  // this constrained instance, and (2) memory pressure at the moment Chromium launches.
  console.log("[DEBUG pdf-gen] htmlContent length:", htmlContent.length)
  const externalResourceMarkers = ["<link", "<img", "http://", "https://", "fonts.googleapis", "cdn."]
  const foundMarkers = externalResourceMarkers.filter((marker) => htmlContent.includes(marker))
  console.log("[DEBUG pdf-gen] external resource markers found:", foundMarkers.length > 0 ? foundMarkers : "none")
  console.log("[DEBUG pdf-gen] process.memoryUsage():", process.memoryUsage())
  ...
  // --disable-dev-shm-usage avoids Chromium crashing/hanging on Docker's tiny default
  ...
  try {
    ...
    // domcontentloaded, not networkidle0: this is a self-contained HTML string, not a
    // page navigation — networkidle0 waits for ALL network activity (including any
    // external font/CDN/image request the LLM's HTML happens to reference) to go fully
    ...
    // on the same free-tier instance of memory/CPU (the likely reason this was reported
    // as failing twice in a row, not just once).
    await browser.close()
  }
}
